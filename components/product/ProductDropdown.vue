...
const PLACEHOLDER_IMAGE = 'data:image/svg+xml;charset=UTF-8,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 80 80%22%3E%3Crect width=%2280%22 height=%2280%22 rx=%2212%22 fill=%22%23f8fafc%22/%3E%3Cpath d=%22M24 52l10-10l8 8l14-14l8 16H24z%22 fill=%22%23cbd5e1%22/%3E%3Ccircle cx=%2231%22 cy=%2230%22 r=%226%22 fill=%22%23cbd5e1%22/%3E%3C/svg%3E';
...
const rows = computed<DropdownRow[]>(() =>
  props.products.flatMap((group) =>
    group.menu_sub.flatMap((sub) =>
      sub.menu_dtl.map((detail) => {
        const stock = Math.max(detail.item_no ?? 0, 0);
        const isInStock = detail.menu_dtl_alive === 'T' && stock > 0;
        const basePrice = stock > 0 ? stock * 100 : 0;
        const name = toText(detail.menu_dtl_name);

        return {
          id: detail.menu_dtl_id,
          sku: toText(detail.slug_dtl),
          name,
          brand: toText(group.group_name),
          price: basePrice,
          category: toText(sub.grp),
          image: PLACEHOLDER_IMAGE,
          specs: toText(sub.grpname),
          promotion: NO_DATA,
          warranty: NO_DATA,
          delivery: NO_DATA,
          views: NO_DATA,
          ...
          sortKey: `${group.group_name}-${sub.grp}-${detail.menu_dtl_name}`,
        };
      }),
    ),
  ),
...
const roleMultiplier = computed(() => getRoleMultiplier(currentUser.value?.role));

const rolePriceById = computed<Record<number, number>>(() =>
  rows.value.reduce((acc, product) => {
    acc[product.id] = product.price * roleMultiplier.value;
    return acc;
  }, {} as Record<number, number>),
);

const rolePrice = (product: DropdownRow) => rolePriceById.value[product.id] ?? product.price;
  ...
  <div class="w-full mb-1">
    ...
    >
      <div class="flex space-x-1 w-full text-sm items-center">
        <span class="text-slate-800 font-bold">{{ label || activeSubTag || NO_DATA }}</span>
    ...
    >
      <div v-if="isOpen" class="mt-2 bg-slate-50 border border-slate-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          <table class="w-full min-w-300 text-left border-separate border-spacing-0 table-fixed">
            <thead class="bg-slate-50 sticky top-0 z-30">
              <tr class="text-[12px] uppercase tracking-tighter text-slate-500 font-black">
                ...
                <th class="p-3 w-25 text-center bg-blue-100/30 text-blue-900 border-b border-r border-slate-200">
                  SRP
                </th>
                <th class="p-3 w-30 text-center bg-[#0D95DA]/10 text-[#0D95DA] border-b border-r border-slate-200 uppercase">
                  {{ currentUser?.role || 'Guest' }}
                </th>
                <th class="p-3 w-20 text-center bg-slate-50 border-b border-r border-slate-200">
                  จำนวน
                </th>
                <th class="p-3 w-27.5 bg-emerald-50 text-center text-emerald-700 border-b border-r border-slate-200">
                  6 ชิ้น+
                </th>
                <th class="p-3 w-30 text-center bg-slate-50 border-b border-slate-200 sticky right-0 z-30 border-l-2 border-l-slate-300 shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                  ตัวเลือก
                </th>
              </tr>
            </thead>
            ...
            <tbody class="text-[12px] bg-white">
              ...
              >
                ...
                <td class="p-3 font-bold md:sticky left-20 z-20 bg-white group-hover:bg-slate-50 border-b border-r border-slate-200 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                  <NuxtLink :to="getProductUrl(product)" class="hover:scale-105 active:scale-95">
                    ...
                    <div class="gap-1 flex flex-wrap mt-1">
                      ...
                      <span
                        v-else
                        class="px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest bg-slate-100 text-slate-500 border border-slate-200 opacity-75"
                      >
                        {{ product.statusLabel }}
                      </span>
                    </div>
                ...
                <td class="p-3 border-b border-r border-slate-100 whitespace-normal text-slate-500 italic">
                ...
                <td class="p-3 text-center font-bold bg-slate-50/50 border-b border-r border-slate-100 text-slate-400">
                  ฿{{ formatNumber(product.price) }}
                </td>

                <td class="p-3 text-center font-black bg-blue-50 text-[#0D95DA] border-b border-r border-blue-100">
                  ฿{{ formatNumber(rolePrice(product)) }}
                </td>

                <td class="p-3 border-b border-r border-slate-100 text-center">
                  <input
                    v-model.number="cartQuantities[product.id]"
                    type="number"
                    min="1"
                ...
                <td class="p-3 text-center font-bold bg-emerald-50 text-emerald-800 border-b border-r border-emerald-100">
                  ฿{{ formatNumber(rolePrice(product) * 0.98) }}
                </td>

                <td class="p-3 text-center border-b sticky right-0 z-20 bg-white group-hover:bg-slate-50 border-l-2 border-l-slate-300 shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                  <button
                    v-if="product.stock && product.stock > 0"
                  ...
                  >แจ้งเตือน<br>เมื่อมีสินค้าเข้า</span>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div v-else class="p-4 md:p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
          <ProductCard v-for="product in rows" :key="product.id" :product="product" view-mode="grid" />
        </div>
      </div>
    </Transition>
    <ModalStockNotify
