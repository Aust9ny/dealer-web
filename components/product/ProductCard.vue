<template>
  ...
  >
    ...
    >
      ...
      <div class="absolute top-1 right-1 z-20 flex flex-col items-end gap-1 max-w-[70%]">
        ...
        <div class="bg-[#facc15] text-[8px] font-black px-2 py-0.5 rounded-l-full shadow-sm flex items-center gap-1">
          <span class="bg-white text-[#facc15] rounded-full w-2.5 h-2.5 flex items-center justify-center text-[6px]">▶</span>
          PARTNER PROMO
        </div>
    ...
    <div class="space-y-1 grow flex flex-col">
      ...
      <div class="mt-3 min-h-10.5">
        <div class="border-2 border-dashed border-[#3b82f6]/30 rounded-lg p-2 bg-[#f0f7ff]">
    ...
    <div class="mt-4 pt-4 border-t border-slate-50 relative min-h-28.75">
      <div class="flex items-start justify-between mb-4">
        <div class="flex flex-col">
          <span class="text-xl font-black text-slate-900 leading-none min-h-6">
            ฿{{ formatNumber(product.price) }}
          </span>
        </div>

        <div class="flex flex-col gap-1 items-end min-h-8">
          <span
      ...
      <div class="min-h-10 flex items-center">
        <button
          v-if="viewMode === 'grid'"
          class="w-full bg-[#0D95DA] text-white py-2 rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#0b7cb5] transition-all active:scale-95 shadow-sm"
        >
          <svg
        ...
        <div v-else class="flex justify-center w-full gap-1">
          ...
          <span class="text-xs text-slate-400">{{ product.views || 0 }}</span>
        </div>
      </div>
    </div>
  </article>
