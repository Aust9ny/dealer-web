...
const handleKeyPressNumeric = (event: KeyboardEvent) => {
...
<template>
  <div class="flex flex-col w-full">
    ...
    <div class="grid grid-cols-12 gap-6 items-start">
      <div class="col-span-8 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <button
              ...
              <Icon
                ...
                class="w-10 h-10 text-slate-300 group-hover:text-[#0D95DA] transition-colors mb-2" />
              <span
                class="text-xs font-black text-slate-400 group-hover:text-slate-600 uppercase tracking-widest">เพิ่มที่อยู่จัดส่ง</span>
            </button>
            ...
            <button
              ...
              <Icon
                ...
                class="w-10 h-10 text-slate-300 group-hover:text-[#0D95DA] transition-colors mb-2" />
              <span
                class="text-xs font-black text-slate-400 group-hover:text-slate-600 uppercase tracking-widest">เลือกวิธีการจัดส่ง</span>
            </button>
          </div>
          <p class="text-[10px] text-red-500 mt-6 font-bold">* กรณีสั่งซื้อสินค้าจำนวน 3 รายการ "มูลค่ารวม 10,000
            บาทขึ้นไป" สามารถเลือกส่งด่วนได้</p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm">
          ...
          <div v-if="currentTaxAddress" class="border rounded-2xl p-6 bg-slate-50/50 relative">
            <div class="flex justify-between items-center mb-4">
              <span
                class="font-bold text-slate-500 text-xs uppercase tracking-widest text-shadow">ข้อมูลใบกำกับภาษี:</span>
              <button class="text-[#0D95DA] text-xs font-bold underline hover:text-blue-700" @click="openModal('tax')">
                แก้ไข
              </button>
            </div>
            ...
            <p class="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-tight">
              เลขประจำตัวผู้เสียภาษี: 0107565000XXX
            </p>
          </div>
          ...
          <button
            ...
            <div
              class="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center mb-3 group-hover:bg-blue-50 transition-colors">
          ...
          <div class="grid grid-cols-2 gap-4">
            <button
              ...
              :class="selectedPayment === 'bank'
                ...
                : 'border-slate-100 text-slate-500 hover:bg-slate-50'" @click="selectedPayment = 'bank'">
              <div class="absolute top-4 left-4">
                <Icon
                  :icon="selectedPayment === 'bank' ? 'mdi:radiobox-marked' : 'mdi:radiobox-blank'" class="w-5 h-5"
            ...
            <button
              ...
              :class="selectedPayment === 'qr'
                ? 'border-[#0D95DA] bg-blue-50/50 text-[#0D95DA]'
                : 'border-slate-100 text-slate-500 hover:bg-slate-50'" @click="selectedPayment = 'qr'">
              <div class="absolute top-4 left-4">
                <Icon
                  :icon="selectedPayment === 'qr' ? 'mdi:radiobox-marked' : 'mdi:radiobox-blank'" class="w-5 h-5"
      ...
      <div class="col-span-4 sticky top-24">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden relative StyledReceipt">
          <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-[12px] text-slate-400 font-bold uppercase tracking-widest">{{ formatDate(po?.createdAt ||
                  '') }}</p>
              </div>
            </div>
            ...
            <div class="space-y-3 pt-2">
              ...
              <div
                ...
                <div class="flex items-center gap-2">
                  <span>ส่วนลด:</span>
                  <span
                    class="bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200 text-[10px] font-black italic tracking-widest uppercase">#DEALER2026</span>
                </div>
            ...
            <div class="mt-4 border-t-2 border-black">
              <div class="bg-blue-50/30 p-6 flex justify-between items-center rounded-b-2xl">
                <div class="flex flex-col">
                  <span
                    class="text-base font-black text-slate-800 uppercase tracking-tighter">ยอดรวมสุทธิทั้งหมด:</span>
                  <span class="text-[10px] text-slate-400 font-medium italic">(ราคานี้รวมภาษีมูลค่าเพิ่ม / Vat
                    แล้ว)</span>
                </div>
          ...
          <div
            class="absolute bottom-0 left-0 w-full h-4 bg-[radial-gradient(circle,transparent_8px,#fff_8px)] bg-[length:24px_24px] bg-repeat-x -mb-2 z-10" />
        </div>
      </div>
    </div>
    ...
    <AddressSelectionModal
      ...
      @keypress-numeric="handleKeyPressNumeric"
