...
import { useCategoryFilters } from '~/composables/category/useCategoryFilters';
...
<template>
  <div class="flex flex-col min-h-screen w-full bg-slate-100 p-2 md:p-4 font-sans">
          ...
          <div v-for="cat in categories" :key="cat.id" class="flex flex-col">
            <div :aria-label="$t('aria.select_sub_cat', { name: cat.name })" class="p-3 rounded-xl cursor-pointer flex items-center transition-all group m-1" :class="[activeCategory === cat.id ? 'text-primary font-bold bg-blue-50/50' : 'text-slate-900 hover:bg-slate-50', !isSidebarOpen ? 'justify-center p-2' : 'justify-between']" @click="toggleCategory(cat, $event)">
              <div class="flex items-center w-full" :class="[!isSidebarOpen ? 'justify-center' : 'gap-3']">
                ...
                <span v-if="isSidebarOpen" class="font-medium whitespace-nowrap text-sm">{{ cat.name }}</span>
              </div>
        ...
        <section class="mt-4 space-y-6 pb-20">
          ...
          <template v-else>
            <div v-for="(taggedProducts, tagName) in productsBySubTag" :key="tagName">
              <ProductDropdown :label="tagName" :products="taggedProducts" :view-mode="viewMode" :is-open="dropdownStates[tagName]" @toggle="toggleDropdown(tagName)" />
            </div>
          </template>
        </section>
      </main>
    ...
    <Transition name="fade">
      <div v-if="isQuickSelectOpen" class="fixed inset-0 z-200" :class="[$viewport.isGreaterOrEquals('lg') ? 'pointer-events-none' : 'flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md']">
        <div class="absolute inset-0 pointer-events-auto" @click="isQuickSelectOpen = false" />
        <div class="bg-white w-full shadow-2xl z-10 flex flex-col overflow-hidden transition-all duration-300 pointer-events-auto border border-slate-200" :class="[$viewport.isGreaterOrEquals('lg') ? 'fixed rounded-4xl w-85' : 'relative rounded-[2.5rem] min-h-125 max-h-[82vh]']" :style="desktopStyles">
          <div class="p-5 border-b bg-slate-50/50 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
              <button v-if="tempCategory && !$viewport.isGreaterOrEquals('lg')" :aria-label="$t('aria.back_to_main_cat')" class="p-2 bg-white rounded-full shadow-sm text-[#0D95DA] active:scale-90 transition-transform" @click="tempCategory = null">
          ...
          <div class="flex-1 p-3 overflow-y-auto space-y-1 scrollbar-hide bg-white">
            ...
            <div v-else class="space-y-1">
              <button v-for="sub in tempCategory.subCats" :key="sub" :aria-label="$t('aria.select_sub_cat', { name: sub })" class="w-full p-3.5 rounded-2xl text-left flex items-center justify-between transition-all group border border-transparent" :class="activeSubCategory === sub ? 'bg-blue-50 text-[#0D95DA] font-bold border-blue-100' : 'hover:bg-slate-50 text-slate-600'" @click="handleModalSelect(tempCategory.id, sub)">
                ...
                <Icon :icon="activeSubCategory === sub ? 'mdi:check-circle' : 'mdi:chevron-right'" class="w-4 h-4" :class="activeSubCategory === sub ? 'text-[#0D95DA]' : 'text-slate-300'" />
              </button>
            </div>
          </div>
