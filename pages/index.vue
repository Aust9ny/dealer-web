            ...
            <div class="mt-10">
              ...
              <div class="grid grid-cols-1 gap-4 mt-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                  ...
                  <div class="bg-white rounded-2xl p-4 border-l-4 border-emerald-500 shadow-sm border-y border-r ">
                    <div class="flex items-center justify-between mb-4">
                      <h4 class="text-xs font-black text-slate-800 uppercase tracking-tighter">🔥 Most Sales</h4>
                      <span class="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded">Top
                        5</span>
                    </div>

                    <div class="space-y-2">
                      <div v-for="(product, i) in topSales" :key="i" class="flex items-center gap-3 group">
                        <span class="text-[10px] font-bold text-slate-300 group-hover:text-emerald-500 w-3">{{ i + 1
                          }}</span>
                        <p class="flex-1 text-[11px] font-bold text-slate-600 truncate">{{ product.name }}</p>
                        <span class="text-[11px] font-black text-slate-800 whitespace-nowrap">฿{{ (product.total /
                          1000).toFixed(0)
                          }}k</span>
                      </div>
                    </div>
                  ...
                  <div class="bg-white rounded-2xl p-4 border-l-4 border-rose-500 shadow-sm border-y border-r">
                    <div class="flex items-center justify-between mb-4">
                      <h4 class="text-xs font-black text-slate-800 uppercase tracking-tighter">❄️ Least Sales</h4>
                      <span class="text-[10px] text-rose-600 font-bold bg-rose-50 px-2 py-0.5 rounded">Bottom 5</span>
                    </div>

                    <div class="space-y-2">
                      <div v-for="(product, i) in bottomSales" :key="i" class="flex items-center gap-3 group">
                        <span class="text-[10px] font-bold text-slate-300 group-hover:text-rose-500 w-3">{{ i + 1
                          }}</span>
                        <p class="flex-1 text-[11px] text-slate-400 italic truncate">{{ product.name }}</p>
                        <span class="text-[11px] font-bold text-slate-400 whitespace-nowrap">{{ product.amount }}
                          pcs</span>
                      </div>
                    </div>
                  </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div
           v-for="(item, index) in quickAccessItems" :key="index"
            ...
            <span class="font-black text-xl tracking-tight" :class="item.textColor">
              {{ item.title }}
            </span>

            <div class="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl" />
          </div>
